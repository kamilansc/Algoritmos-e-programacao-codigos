...
def ler_num_int_positivo(label):
    numero = ler_num_int(label)
    if numero > 0:
        return numero
    else:
        print(f'Ops! O número ("{numero}") digitado não é válido como inteiro positivo. Tente novamente!')
        return ler_num_int_positivo(label)
